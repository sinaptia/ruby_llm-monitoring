...
### Using a governed endpoint

`ruby_llm-monitoring` records the RubyLLM events generated by your Rails application. You can also route RubyLLM itself through an OpenAI-compatible gateway when you need centralized policy, audit, routing, quotas, or cost controls across several apps.

For example, with Tuning Engines:
...
RubyLLM continues to emit the events that power this dashboard, while the gateway handles organization-wide controls such as model access, policy checks, and budget limits.
